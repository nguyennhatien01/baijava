...
import domain.Person; // 1. Đã sửa từ domain.persion thành domain.Person
import java.sql.*;
import java.util.*;
...
public class JdbcPerson {
	...
	*/
	public List<Person> select() { // 2. Đã sửa kiểu trả về thành List<Person>
		Connection conn = null;
		PreparedStatement stmt = null;
		ResultSet rs = null;
		Person persona = null;	// 3. Đã sửa thành Person
		List<Person> personas = new ArrayList<>(); // 4. Đã sửa thành List<Person>
		...
		try {
			...
			while (rs.next()) {
				...
				persona = new Person(); // 5. Đã sửa thành new Person()
